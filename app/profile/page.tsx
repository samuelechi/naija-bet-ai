        ...
        <div style={{ maxWidth: '430px', margin: '0 auto', minHeight: '100vh' }}>
            <main className="flex flex-col min-h-screen" style={{ background: '#0A0A0F' }}>
                ...
                <div className="relative pt-14 pb-6 px-5 overflow-hidden">
                    ...
                    <div className="relative flex flex-col items-center gap-3">
                        <div className="w-18 h-18 rounded-2xl flex items-center justify-center text-xl font-black text-white"
                            style={{ background: 'linear-gradient(135deg, #166534, #15803d)', border: '1px solid rgba(74,222,128,0.2)', boxShadow: '0 0 30px rgba(22,163,74,0.2)', width: 72, height: 72 }}>
                            {initials}
                        </div>
                        ...
                        <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-full"
                            style={{ background: isPro ? 'rgba(22,163,74,0.1)' : 'rgba(100,100,100,0.1)', border: isPro ? '1px solid rgba(34,197,94,0.2)' : '1px solid rgba(150,150,150,0.2)' }}>
                ...
                <div className="flex-1 px-5 space-y-2 pb-28">
                    ...
                    <MenuItem icon="💳" iconBg="rgba(34,197,94,0.1)" title="Subscription"
                        subtitle={isPro ? 'Pro Plan · Active' : 'Free Plan · Upgrade now'}
                        onClick={() => router.push('/subscribe')} />
                    <MenuItem icon="📊" iconBg="rgba(56,189,248,0.1)" title="Prediction History"
                    ...
                    >
                        <div className="w-9 h-9 rounded-xl flex items-center justify-center text-base shrink-0"
                            style={{ background: 'rgba(251,191,36,0.1)', border: '1px solid rgba(251,191,36,0.15)' }}>🔔</div>
                        <div className="flex-1 min-w-0">
                            <p className="text-white text-sm font-bold">Notifications</p>
                            <p className="text-slate-500 text-[10px] mt-0.5">{notifications ? 'Pre-match alerts on' : 'Notifications off'}</p>
                        </div>
                        <div className="shrink-0 w-11 h-6 rounded-full relative transition-colors duration-200"
                    ...
                    <div>
                        ...
                        >
                            <div className="w-9 h-9 rounded-xl flex items-center justify-center text-base shrink-0"
                                style={{ background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.15)' }}>⚽</div>
                            <div className="flex-1 min-w-0">
                                ...
                                {ALL_LEAGUES.map(league => (
                                    <button key={league} onClick={() => toggleLeague(league)}
                                        ...
                                        style={selectedLeagues.includes(league) ? {
                                            ...
                                            background: '#1a1a24', color: '#4b5563', border: '1px solid rgba(255,255,255,0.06)',
                                        }}>
                                        {league}
                                    </button>
                                ))}
                    ...
                    {!isPro && (
                        <MenuItem icon="⭐" iconBg="rgba(34,197,94,0.1)" title="Upgrade to Pro"
                    ...
                    >
                        <div className="w-9 h-9 rounded-xl flex items-center justify-center text-base shrink-0"
                            style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.15)' }}>🚪</div>
                    ...
                    <div
                        ...
                        className="flex items-center gap-3 px-4 py-3.5 rounded-2xl cursor-pointer active:scale-[0.98] transition-transform"
                        style={{ background: '#111118', border: '1px solid rgba(239,68,68,0.08)' }}
                    >
                        <div className="w-9 h-9 rounded-xl flex items-center justify-center text-base shrink-0"
                            style={{ background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.12)' }}>🗑️</div>
                        <div className="flex-1">
                            <p className="text-red-500/70 text-sm font-bold">Delete Account</p>
                            <p className="text-slate-600 text-[10px] mt-0.5">Permanently remove your data</p>
                        </div>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#475569" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                            <polyline points="9 18 15 12 9 6" />
                        </svg>
                ...
                {showDeleteConfirm && (
                    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: '0 16px 32px' }}>
                        <div style={{ background: '#111118', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 24, padding: '28px 24px', width: '100%', maxWidth: 400 }}>
                            <div className="text-center mb-6">
                                ...
                                <p className="text-slate-500 text-sm leading-relaxed">
                                    This will permanently delete your account, all predictions, and history. This cannot be undone.
                                </p>
                            </div>
                            <div className="flex gap-3">
                                <button
                                    onClick={() => setShowDeleteConfirm(false)}
                                    className="flex-1 py-3.5 rounded-xl font-black text-sm text-white"
                                ...
                                <button
                                    ...
                                    disabled={deleting}
                                    className="flex-1 py-3.5 rounded-xl font-black text-sm text-white"
                                    style={{ background: 'linear-gradient(135deg, #dc2626, #ef4444)', opacity: deleting ? 0.6 : 1 }}
                                >
...
function SectionLabel({ children }: { children: React.ReactNode }) {
    return (
        <p className="text-slate-600 text-[9px] uppercase tracking-[0.15em] font-bold px-1 pt-3 pb-1">{children}</p>
    )
}

function MenuItem({ icon, iconBg, title, subtitle, onClick }: {
    icon: string; iconBg: string; title: string; subtitle: string; onClick: () => void
}) {
    return (
        <div onClick={onClick}
            className="flex items-center gap-3 px-4 py-3.5 rounded-2xl cursor-pointer active:scale-[0.98] transition-transform"
